...
class EasyTraserGUI:
    ...
    def download_data(self):
        ...
        if not symbol:
            messagebox.showerror("输入错误", "请填写股票代码")
            return
        # 自动补全后缀
        ...
        try:
            download_stock_to_sqlite(symbol_full, start, end)
            # 检查数据是否真的下载成功
            from trader.data_downloader import load_stock_from_sqlite
            df_check = load_stock_from_sqlite(symbol_full)
            if df_check is not None and not df_check.empty:
                self.text.insert(tk.END, f"{symbol_full} {start}~{end} 数据下载并保存到本地数据库成功！\n")
            else:
                messagebox.showerror("下载失败", f"{symbol_full} 没有获取到有效数据，请检查代码或网络。")
    ...
    def load_data_sqlite(self):
        ...
        try:
            ...
            self.symbol = symbol
            self.text.insert(tk.END, f"{symbol} 数据加载成功，{len(self.df)} 条记录\n")
        except Exception as e:
    ...
    def run_ai(self):
        ...
        try:
            ...
            self.text.insert(tk.END, f"AI预测结果（前5条）：{preds[:5]}\n")
        except Exception as e:
